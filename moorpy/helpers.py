...
def dsolve2(eval_func, X0, Ytarget=[], step_func=None, args=[], tol=0.0001, ytol=0, maxIter=20, 
    ...
    for iter in range(maxIter):
        ...
        #         position_translated = X[0:3] + Rmat @ - (heightlist[i]-1)
    ...
    for iter in range(maxIter):
        ...
        if len(dX) >= 7:
            ...
            for i in range(len(dX)//6 - 1):
                ...
                #print(dX)
                Rmat = rotationMatrix(*X[3:6])
                nodeup = dX[(i+1)*6:(i+1)*6+3] + np.array([0,0,heightlist[i+1]])
                #nodeup[2] += heightlist[i+1]n
                nodedown = dX[0:3] + np.array([0,0,15])
                #nodedown[2] += 15

                dist = nodeup-nodedown
                #print('Moorpy NODES', nodeup)
                #print(nodedown)
                #print(dist)
                position_translated = X[0:3] + Rmat @ (dist) - (dist)

                #print('position_translated ', position_translated )
                #position_translated = Rmat @ - (self.towerra+r6[0:3]) + (self.towerra+r6[0:3])
                #print('r6input', r6)
                #print('Toweerra', self.towerra)
                #Construct new r6 with updated position but same orientation
