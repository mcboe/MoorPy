...
def TLP(XF, YF, ZF, L, EA, W, CB=0, alpha=0, HF0=0, VF0=0, Tol=0.000001, 
    ...
    #                     Te[I] = W*(s[I] - LA)
        ...
        # else:
        L = 75
        T = np.sqrt(YF0**2 + XF0**2+VF0**2)
        #delta_L = (np.sqrt(HF0**2 + VF0**2)-T0)/(EA/L)
        delta_Lx = np.sqrt(XF**2 + ZF**2)-75
        delta_Ly = np.sqrt(YF**2 + ZF**2)-75
        delta_Ltot = np.sqrt(YF**2 + XF**2 + ZF**2)
        if delta_L >= 0:
            HFx = T0*(XF/(L+delta_Lx)) + EA*(delta_Lx/L)*(XF/(L+delta_Lx))
            HFy = T0*(YF/(L+delta_Ly)) + EA*(delta_Ly/L)*(YF/(L+delta_Ly))
            VF = T0*(ZF/(L+delta_Ltot)) + EA*(delta_Ltot/L)*(ZF/(L+delta_Ltot))
        #else:
        #    HF = T0*(XF/(L+delta_L)) + EA*(delta_L/L)*(XF/(L+delta_L))
        #    VF = T0*(ZF/(L+delta_L)) + EA*(delta_L/L)*(ZF/(L+delta_L))
        ...
        #print('VFO', VF0, HF0)
        L_stretchedtot = L + delta_Ltot  # Total stretched length
        L_stretchedtotx = L + delta_Lx
        L_stretchedtoty = L + delta_Ly
        ...
        K31 = 0.5* (T0/L_stretched * ((ZF / L_stretched)-1 ) + EA/L_stretched * (ZF / L_stretched))
        #
        print('Values')
        print(T0)
...
def catenary(XF, XFnl, YFnl, ZF, L, phiz, EA, W, CB=0, alpha=0, HF0=0, VF0=0, Tol=0.000001, 
             nNodes=20, MaxIter=100, plots=0, depth=0, T0=0):
    ...
    #                     Te[I] = W*(s[I] - LA)
        ...
        info['Zextreme'] = 0  # No slack portion
        info["stiffnessK31"] = K31
        info["stiffnessK32"] = K32
        info["stiffnessK36"] = K36
